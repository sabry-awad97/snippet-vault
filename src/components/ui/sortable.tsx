...
interface SortableProps<TData extends { id: UniqueIdentifier }>
   ...
   */
  overlay?: React.ReactNode | ((item: TData | null) => React.ReactNode) | null;
}
...
function Sortable<TData extends { id: UniqueIdentifier }>({
  ...
  const [activeItem, setActiveItem] = React.useState<TData | null>(null);
  ...
  return (
    <DndContext
      ...
      onDragStart={({ active }) => {
        ...
        const item = value.find(item => item.id === active.id) ?? null;
        setActiveItem(item);
      }}
    ...
    >
      ...
      {overlay && (
        <SortableOverlay activeId={activeId}>
          {typeof overlay === 'function' ? overlay(activeItem) : overlay}
        </SortableOverlay>
      )}
    </DndContext>
  );
